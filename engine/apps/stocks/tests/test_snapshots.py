"""Golden-snapshot tests for the stocks app."""
...
CASES = harness.all_cases("stocks")
...
def test_snapshot(fixture_id: str, w: int, h: int, snapshot_update: bool) -> None:
    result = harness.render_case("stocks", fixture_id, w, h)
    assert result.image.size == (w, h)
    assert_snapshot(result.image, "stocks", harness.case_id(fixture_id, w, h), snapshot_update)
