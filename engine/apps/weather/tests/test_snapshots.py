"""Golden-snapshot tests for the weather app."""
...
CASES = harness.all_cases("weather")
...
def test_snapshot(fixture_id: str, w: int, h: int, snapshot_update: bool) -> None:
    result = harness.render_case("weather", fixture_id, w, h)
    assert result.image.size == (w, h)
    assert_snapshot(result.image, "weather", harness.case_id(fixture_id, w, h), snapshot_update)
