"""Golden-snapshot tests for the text app."""
...
def test_snapshot(fixture_id: str, w: int, h: int, snapshot_update: bool) -> None:
    result = harness.render_case("text", fixture_id, w, h)
    assert result.image.size == (w, h)
    assert_snapshot(result.image, "text", harness.case_id(fixture_id, w, h), snapshot_update)
