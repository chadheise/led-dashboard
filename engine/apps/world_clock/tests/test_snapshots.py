"""Golden-snapshot tests for the world_clock app."""
...
CASES = harness.all_cases("world_clock")
...
def test_snapshot(fixture_id: str, w: int, h: int, snapshot_update: bool) -> None:
    result = harness.render_case("world_clock", fixture_id, w, h)
    assert result.image.size == (w, h)
    assert_snapshot(result.image, "world_clock", harness.case_id(fixture_id, w, h), snapshot_update)
